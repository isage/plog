...
int CallImeDialog(SceImeDialogParam *param)
{
  ...
  param->sdkVersion = 0x03150021, ret = sceImeDialogInit(param);
...
int SetServerPort(void)
{
  ...
  if (atoi(ServerPortStrUtf8) < 1 || atoi(ServerPortStrUtf8) > UINT16_MAX)
  {
...
int QafSettings(void)
{
  ...
  while (1)
  {
    ...
    psvDebugScreenPrintf2(20, 20, "kernel debug print : %s",
                          (NetLoggingMgrConfig.flags & NLM_CONFIG_FLAGS_BIT_QAF_DEBUG_PRINTF) ? "Enable" : "Disable");
